...
const serviceAccount = {
  ...
  auth_provider_x509_cert_url:
    process.env.FIREBASE_MENU_KITA_PROVIDER_X509_CERT_URL || '',
  client_x509_cert_url:
    process.env.FIREBASE_MENU_KITA_CLIENT_X509_CERT_URL || '',
  universe_domain: process.env.FIREBASE_MENU_KITA_UNIVERSE_DOMAIN || '',
};

firebaseConfig.initializeApp({
  credential: firebaseConfig.credential.cert(JSON.stringify(serviceAccount)),
});
